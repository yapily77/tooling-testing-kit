...
async def test_handle_forecast_category_no_scored_to_dicts_nameerror():
    ...
    """
    with patch("src.interfaces.telegram.chronomancer.coordinator.get_session") as mock_get_session, \
         patch("src.interfaces.telegram.chronomancer.coordinator._reconstruct_session_profile", return_value=True), \
         patch("src.interfaces.telegram.chronomancer.coordinator._session_to_profile", return_value={}), \
         patch("src.interfaces.telegram.chronomancer.forecast_store.get_rolling_30") as mock_get_rolling_30:
...
async def test_intake_handle_start_command_no_do_start_nameerror():
    ...
    """
    with patch("src.interfaces.telegram.intake.intake.save_session"), \
         patch("src.interfaces.telegram.intake.intake.get_session") as mock_get_session:
        mock_session = MagicMock()
