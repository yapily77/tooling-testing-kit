...
2. **Understand the prompt and ask:** Read `TEST/fuzzing/SKILL.md`. Locate and review the relevant target modules in `src/` before writing tests.
3. **Perform tasks:** 
   - Write the advanced Hypothesis fuzzing test in the `TEST/fuzzing/` directory.
   - Run `uv run pytest TEST/fuzzing/<target_file> -q`.
   - **CRITICAL:** If the fuzzer breaks the system (e.g., timezone mismatch, state machine hang, or tokenizer unhandled exception), the agent MUST go into `src/` and fix the source code to patch the vulnerability until the fuzzer passes.
...
**Reference:** `src/engine/daily_pillar.py` or solar/lunar calendar modules.
**Task Details:**
...
**Reference:** `src/bot/` (Telegram router / conversational states).
**Task Details:**
...
**Reference:** Text chunking utilities and embedding ingestion (e.g., `BGEM3`, `Qdrant` wrappers in `src/`).
**Task Details:**
