...
> [!NOTE]
> **Test Optimization**: Because `04_monthly` executes the full auto-intake sequence (Steps 1–7) as a prerequisite to saving the user profile in the database, running `04_monthly` already fully validates the `02_auto` flow. If you run `04_monthly`, there is no need to run `02_auto` separately.
...
Before initiating the sequential monthly forecast compilation, a diagnostic verification is executed automatically at runtime inside `pipeline_check.py`.
...
All LLM inputs and outputs are captured automatically for training and analysis via a custom OpenTelemetry `SpanProcessor` registered in `app.py`.

* **Output Directory**: `logs/training_data/`
