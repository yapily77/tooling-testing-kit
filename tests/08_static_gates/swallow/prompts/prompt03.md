...
* **Skill Dependency:** All agents MUST read and load `tests/08_static_gates/swallow/SKILL.md` before writing a single line of code.
...
**Target Scope:** `src/engine/` (Strictly the deterministic math modules)
**Task Details:**
...
* **Fixing Fallout:** Run the script. For every flagged line in `src/engine/`, replace `.get("key", default)` with strict bracket notation `["key"]`. If this causes tests to fail due to missing keys, you must fix the data schemas upstream to ensure the key is always populated.
