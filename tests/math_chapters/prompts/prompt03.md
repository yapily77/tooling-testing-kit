...
## Environment Context
* **Test Directory:** `tests/math_chapters/`
* **Reference File:** `tests/math_chapters/test_ch03_production_control.py` (Append tests here or create `tests/math_chapters/test_ch03_edge_cases.py`)
* **Source Modules:** `src/engine/module2_root.py`, `src/engine/module3_interaction.py`, `src/engine/bazi_data.py`
* **Skill Dependency:** All agents MUST read and strictly adhere to `tests/math_chapters/SKILL.md` (Bazi Testing Conventions) before writing a single line of code. Absolutely NO Chinese characters; enforce English CapitalCase for all elements/stems/branches, and use pure Pydantic models.
...
2. **Understand the target:** Read the relevant source code in `src/engine/` and the existing tests in `TEST/math/`. 
3. **Perform tasks:** 
   - Write the Pytest unit test(s) in the `TEST/math/` directory.
   - Run `uv run pytest TEST/math/<target_file> -q`.
   - **CRITICAL:** If the test exposes a bug or crashes the engine, the agent MUST go into `src/engine/` and fix the source code to patch the vulnerability until the test passes.
...
**Reference Functions:** `get_suspended_stems` in `src/engine/module3_interaction.py` and `calculate_dm_strength_tier1` in `src/engine/module2_root.py`
**Task Details:**
...
**Target File:** `TEST/math/test_ch03_edge_cases.py`
**Reference Function:** `calculate_dm_strength_tier1` and `classify_dm_strength` in `src/engine/module2_root.py`
**Task Details:**
...
**Target File:** `TEST/math/test_ch03_edge_cases.py`
**Reference Function:** `calculate_clash_adjusted_dm_score` in `src/engine/module2_root.py` and `get_ten_god`
**Task Details:**
