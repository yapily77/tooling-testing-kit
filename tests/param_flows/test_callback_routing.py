...
async def test_lang_selection(mock_db, mock_session, lang_choice):
    ...
    with patch("src.interfaces.telegram.app.db", mock_db), \
        ...
        from src.interfaces.telegram.app import _handle_lang_callback
...
async def test_start_menu(mock_db, mock_session, mode_choice):
    ...
    with patch("src.interfaces.telegram.app.db", mock_db), \
         ...
         patch("src.interfaces.telegram.session.save_session"), \
         patch("src.interfaces.telegram.utils.answer_telegram_callback", new_callable=AsyncMock) as mock_answer, \
         patch("src.interfaces.telegram.app.check_user_access", return_value=True), \
...
async def test_chart_7day_callback(mock_db, mock_session):
    ...
    with patch("src.interfaces.telegram.app.db", mock_db), \
         ...
         patch("src.interfaces.telegram.session.get_session", return_value=mock_session), \
         patch("src.interfaces.telegram.utils.answer_telegram_callback", new_callable=AsyncMock) as mock_answer, \
         patch("src.interfaces.telegram.chronomancer.handle_week_chart", new_callable=AsyncMock) as mock_chart:

        mock_chart.return_value = (None, "Chart summary text")

        from src.interfaces.telegram.app import _handle_chart_7day_callback
...
async def test_delete_stakeholder_flow(mock_db, mock_session, delete_action):
    ...
    with patch("src.interfaces.telegram.app.db", mock_db), \
        ...
        if delete_action == "confirm":
            from src.interfaces.telegram.app import _handle_confirm_delete_stakeholder_callback
            ...
            from src.interfaces.telegram.app import _handle_cancel_delete_stakeholder_callback
...
async def test_stakeholder_category_selection(mock_db, mock_session, stakeholder_cat):
    ...
    with patch("src.interfaces.telegram.app.db", mock_db), \
        ...
        from src.interfaces.telegram.app import _handle_add_relation_callback
