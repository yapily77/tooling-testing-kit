...
`dailygit-check.py` is invoked from the git `pre-push` hook (see repo root `.git/hooks/pre-push`). It auto-detects changed Python files in `src/` and `kit-hygiene/`, runs 9 scanners on them, then parses JSON reports for blocking violations.

```bash
# Manual run (scans all of src/ + kit-hygiene/):
...
2. Filters for `.py` files in `src/` or `kit-hygiene/` not in `exceptions.json`.
